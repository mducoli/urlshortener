...
export const actions: Actions = {
	...
	> => {
		...
		if (!validator.success)
			return fail(400, { ok: false, error: formatErrs(validator.error.errors) })
	...
	> => {
		...
		if (!check) return fail(404, { error: 'Link not found' })
	...
	> => {
		...
		// CHECK IF EXIST
		const check = await db.get(form.shortid)
		if (!check) return fail(404, { ok: false, error: 'Link not found' })
